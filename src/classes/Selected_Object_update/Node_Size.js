...
export default function node_size(e) {
    if (window.selectedObject && window.selectedObject.line) {
      window.selectedObject.line.node_size =  parseInt(e.target.value);
  
      window.selectedObject.lineWidth =  parseInt(e.target.value); // Update node color
      for (let j = 0; j < window.selectedObject.line.drawn_node.length; j++) {
        window.selectedObject.line.drawn_node[j].node.size =  parseInt(e.target.value);
      }
      drawAllObjects(); // Redraw canvas with updated color
    } else if (window.selectedObject && window.selectedObject.grid) {
      window.selectedObject.grid.node_size =  parseInt(e.target.value);
      window.selectedObject.node_size =  parseInt(e.target.value); // Update grid color
      for (let j = 0; j < window.selectedObject.grid.drawn_node.length; j++) {
        window.selectedObject.grid.drawn_node[j].node.size =  parseInt(e.target.value);
      }
      drawAllObjects(); // Redraw canvas with updated color
    } else if (window.selectednode && window.selectednode.node) {
      window.selectednode.node.size =  parseInt(e.target.value);
      drawAllObjects();
